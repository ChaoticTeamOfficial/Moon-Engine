...
class MoonUtils
{
	...
	public static inline function spaceToDash(s:String):String return s.replace(" ", "-");

	public static inline function dashToSpace(s:String):String return s.replace("-", " ");

	public static inline function swapSpaceDash(s:String):String return s.contains('-') ? dashToSpace(s) : spaceToDash(s);
	...
	{
		if (Paths.exists(path)) return Paths.getFileContent(path).split("\n").map((line) -> return line.trim());
		else
