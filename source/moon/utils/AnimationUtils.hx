...
class AnimationUtils
{
    ...
    public static function addTextureAtlasAnimation(target:FlxAnimate, anim:AnimationData):Void
    {
        ...
        if (anim.indices != null && anim.indices.length > 0)
        {
            switch (animType)
            {
                case FRAMELABEL: controller.addByFrameLabelIndices(anim.name, anim.prefix, anim.indices, frameRate, looped);
                case SYMBOL: controller.addBySymbolIndices(anim.name, anim.prefix, anim.indices, frameRate, looped);
            }
        }
        else
        {
            switch (animType)
            {
                case FRAMELABEL: controller.addByFrameLabel(anim.name, anim.prefix, frameRate, looped);
                case SYMBOL: controller.addBySymbol(anim.name, anim.prefix, frameRate, looped);
            }
        }
    ...
    {
        if (Std.isOfType(target, MoonSprite))
            cast(target, MoonSprite).animDataMap.set(anim.name, anim);
    }
    ...
    public static function loadSpritesheetGraphic(target:FlxSprite, key:String, frameWidth:Int, frameHeight:Int, from:String = 'images', ?library:String):FlxSprite
        return target.loadGraphic(Paths.image(key, from, library), true, frameWidth, frameHeight);
    ...
    public static function registerDances(target:MoonSprite, anim:AnimationData):Void
    {
        if (anim.name.startsWith("idle-"))
        {
            ...
            if (!target.idleAnimsMap.exists(suffix))
                target.idleAnimsMap.set(suffix, []);
        ...
        if (anim.finishAnim != null)
        {
            target.animation.onFinish.add(finishedName ->
            {
                ...
                final playedName = (target.animationSuffix != "" && cur.endsWith('-${target.animationSuffix}'))
                ...
                if (playedName == anim.name)
                {
                    ...
                    else target.playAnim(anim.finishAnim, true);
                }
            });
        }
    }
    ...
    public static function loadAnimations(target:MoonSprite, animations:Array<AnimationData>, type:AtlasType = SPARROW):Array<String>
    {
        ...
        if (!target.idleAnimsMap.exists(""))
            target.idleAnimsMap.set("", []);
