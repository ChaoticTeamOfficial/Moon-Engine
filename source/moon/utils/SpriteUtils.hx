...
class SpriteUtils
{
    ...
    {
        if(axes == X || axes == XY)
            spriteA.x = spriteB.x + spriteB.width / 2 - spriteA.width / 2;

        if(axes == Y || axes == XY)
            spriteA.y = spriteB.y + spriteB.height / 2 - spriteA.height / 2;
    }
    ...
    static function doAppearAnim(sprite:MoonSprite, anim:AppearAnim, outAnim:DisappearAnim, setTween:FlxTween->Void)
    {
        ...
        switch(anim)
        {
            ...
            case LASER:
                ...
                tween = FlxTween.color(sprite, duration, FlxColor.WHITE, ogCol, {ease: FlxEase.circOut, onComplete: _-> doDisappearAnim(sprite,outAnim,setTween)});

            case SHAKE:
                tween = FlxTween.shake(sprite, 0.03, duration - 0.16, XY, {ease: FlxEase.expoOut, onComplete: _ -> doDisappearAnim(sprite,outAnim,setTween)});
