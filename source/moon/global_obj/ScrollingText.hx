...
class ScrollingText extends MoonSprite
{
	...
	override public function draw():Void
	{
		...
		final sourceRect = new flash.geom.Rectangle(scrollOffset, 0, Math.min(displayWidth, textField.width - scrollOffset), textField.height);

		// stamp the text onto this sprite's canvas
		pixels.copyPixels(textField.updateFramePixels(), sourceRect, new flash.geom.Point(0, 0));
