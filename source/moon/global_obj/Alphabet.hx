...
class Alphabet extends FlxSpriteGroup
{
	...
	static function init():Void
	{
		...
		{
			for (member in Alphabet.alphabetGroup.members) member.destroy();
	...
	{
		for (line in lines) for (char in line) char.kill();
		...
		for (i => text in val.replace('\r', '').split('\n'))
		{
			...
			for (ch in text.split(''))
			{
				...
				if (bold) animName += ' bold';
				else if (letterRegex.match(ch)) animName += animName.toLowerCase() != animName ? ' uppercase' : ' lowercase';
				else
					animName += ' normal';
				...
				if (char.animation.exists('idle')) char.animation.remove('idle');
			...
			if (letterTracker > maxWidth) maxWidth = letterTracker;
		}

		for (i => line in lines) for (char in line) char.y += maxHeight * (i + 1);
	...
	@:noCompletion
	function set_align(val:FlxTextAlign):FlxTextAlign
	{
		if (align == CENTER || align == RIGHT) for (i => line in lines) for (char in line) char.x -= (maxWidth - widths[i]) * (align == CENTER ? .5 : 1);

		if (val == CENTER || val == RIGHT) for (i => line in lines) for (char in line) char.x += (maxWidth - widths[i]) * (val == CENTER ? .5 : 1);

		charPos = [];
		for (i => char in members) charPos[i] = [char.x, char.y];
	...
	override function setColorTransform(redMultiplier = 1.0, greenMultiplier = 1.0, blueMultiplier = 1.0, alphaMultiplier = 1.0, redOffset = 0.0, greenOffset = 0.0, blueOffset = 0.0, alphaOffset = 0.0):Void
		for (member in members) member.setColorTransform(
		redMultiplier,
		greenMultiplier,
		blueMultiplier,
		alphaMultiplier,
