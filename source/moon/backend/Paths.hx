...
class Paths
{
	...
	{
		if (key.endsWith('.png')) key = key.substring(0, key.lastIndexOf('.png'));
	...
	public static function sound(key:String, from:String = 'music', ?library:String):Sound return getSound('$from/$key', library);

	/** Loads a graphic from `assets/<from>/<key>.png`. Defaults to the `images` folder. */
	public static function image(key:String, from:String = 'images', ?library:String):FlxGraphic return getGraphic(key, from, library);
	...
	public static function getSparrowAtlas(key:String, from:String = 'images', ?library:String):FlxAtlasFrames return FlxAtlasFrames.fromSparrow(
		getGraphic(key, from, library),
		getFileContent('$from/$key.xml', library)
	);

	/** Loads a Packer (.txt) atlas from `assets/<from>/<key>.png`. Defaults to the `images` folder. */
	public static function getPackerAtlas(key:String, from:String = 'images', ?library:String):FlxAtlasFrames return FlxAtlasFrames.fromSpriteSheetPacker(
		getGraphic(key, from, library),
		getFileContent('$from/$key.txt', library)
	);

	/** Loads an Aseprite (.json) atlas from `assets/<from>/<key>.png`. Defaults to the `images` folder. */
	public static function getAsepriteAtlas(key:String, from:String = 'images', ?library:String):FlxAtlasFrames return FlxAtlasFrames.fromAseprite(
		getGraphic(key, from, library),
		getFileContent('$from/$key.json', library)
	);
	...
	public static function getMultiSparrowAtlas(baseSheet:String, from:String = 'images', otherSheets:Array<String>, ?library:String):FlxFramesCollection
	{
		...
		for (sheet in otherSheets) for (frame in getSparrowAtlas(sheet, from, library).frames) frames.pushFrame(frame);
		return frames;
	}

	/** Returns a `FlxGraphic` containing a single named frame from a Sparrow atlas. */
	public static function getFrame(key:String, from:String = 'images', frame:String, ?library:String):FlxGraphic return FlxGraphic.fromFrame(
		getSparrowAtlas(key, from).getByName(frame)
	);
	...
	public static function readDir(dir:String, ?typeArr:Array<String>, ?removeType:Bool = true, ?library:String):Array<String>
	{
		...
		{
			for (f in files) if (!seen.exists(f))
			{
				seen.set(f, true);
				tempList.push(f);
			}
		...
		for (mod in Mods.activeMods)
		{
			...
			if (FileSystem.exists(modDir) && FileSystem.isDirectory(modDir)) try
			{
	...
	public inline static function playSFX(key:String, ?from:String = 'sounds', once:Bool = false, ?pitch:Float = 1):FlxSound
	{
		...
		if (!once) return FlxG.sound.play(src, MoonSettings.callSetting('SFX Volume') / 100);
