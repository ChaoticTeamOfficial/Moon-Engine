...
class SongData
{
	...
	static function saveData(songName:String, difficulty:String, mix:String, score:Int, misses:Int, accuracy:Float):Bool
	{
		...
		if (old == null) shouldSave = true;
		else
		{
			if (score > old.score) shouldSave = true;
			else if (score == old.score && misses < old.misses) shouldSave = true;
			else if (score == old.score && misses == old.misses && accuracy > old.accuracy) shouldSave = true;
		}
