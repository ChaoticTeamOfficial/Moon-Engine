...
abstract JudgementsCombo(JudgementsComboFile) from JudgementsComboFile to JudgementsComboFile
{
	...
	{
		if (Paths.exists('images/combo_judgements/$skin/config.json')) return Paths.JSON('images/combo_judgements/$skin/config');
		else
	...
	static function getSpawnList(?exclude:Array<AppearAnim>):Array<AppearAnim>
	{
		...
		if (exclude != null && exclude.length > 0) for (ex in exclude) if (all.contains(ex)) all.remove(ex);
