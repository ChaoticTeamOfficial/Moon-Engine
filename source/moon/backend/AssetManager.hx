...
class AssetManager
{
	...
	{
		if (graphics.exists(cacheKey)) return graphics.get(cacheKey);
	...
	{
		if (sounds.exists(cacheKey)) return sounds.get(cacheKey);
	...
	static function clearGraphics(unusedOnly:Bool):Void
	{
		...
		if (cleared.length > 0) trace('[ASSET-MANAGER] Cleared ${cleared.length} graphic(s): $cleared', "DEBUG");
	}
	...
	static function clearSounds(unusedOnly:Bool):Void
	{
		...
		for (key => snd in sounds)
		{
			...
			if (unusedOnly)
			{
				var inUse = false;
				@:privateAccess
				if (FlxG.sound.music != null && FlxG.sound.music._sound == snd) inUse = true;

				if (!inUse) for (s in FlxG.sound.list) if (@:privateAccess s._sound == snd)
				{
					inUse = true;
					break;
				}
		...
		if (cleared.length > 0) trace('[ASSET-MANAGER] Cleared ${cleared.length} sound(s): $cleared', "DEBUG");
	}
