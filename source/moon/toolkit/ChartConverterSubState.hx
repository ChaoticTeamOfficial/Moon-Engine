...
class ChartConverterSubState extends FlxSubState
{
    ...
    function doConversion()
    {
        ...
        try
        {
            ...
            if (chartData.notes != null)
            {
                ...
                for (rule in noteRules)
                    notes = rule.apply(notes);
            ...
            if (_srtPath != null)
            {
                ...
                catch (e:Dynamic) { setStatus('SRT parse failed: $e', "#f59e0b"); }
                #end
            }
    ...
    function addLabeledDropdown(parent:VBox, label:String, options:Array<String>):DropDown
    {
        ...
        var dd = new DropDown();
        dd.percentWidth = 100;
        for (o in options) dd.dataSource.add({text: o});
...
class NoteRule
{
    ...
    public function new(type:String, onRemove:Void->Void)
    {
        ...
        switch (type)
        {
            case "Retype Notes":
                ...
                var arrow = new Label(); arrow.text = "->"; arrow.styleString = "color: #475569;";
    ...
    public function apply(notes:Array<Dynamic>):Array<Dynamic>
    {
        ...
        {
            case "Retype Notes":
                [for (n in notes) { if (n.type == from) n.type = to; n; }];

            case "Relane Notes":
                [for (n in notes) { if (n.lane == from) n.lane = to; n; }];
