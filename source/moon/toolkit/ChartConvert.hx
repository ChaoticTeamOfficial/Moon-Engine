...
class ChartConvert extends FlxState
{
	...
	function updateStateText()
	{
		switch (state)
		{
			...
			case SavingChart:
				changeTXT('Conversion done! Now save your chart file...');
			case SavingEvents:
				changeTXT('Conversion done! Now save your events file...');
			case Done:
				changeTXT('Conversion done! Enjoy :3', FlxColor.LIME);
				reset();
			case Failed:
	...
	function reset() new FlxTimer().start(5, (_) -> FlxG.resetState());

	@:noCompletion
	public function set_state(state:ConvertState):ConvertState
	{
		this.state = state;
		updateStateText();
		return this.state;
