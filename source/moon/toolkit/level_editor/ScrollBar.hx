...
class ScrollBar extends FlxSpriteGroup
{
	...
	var bookmarks:Array<
		{time:Float, color:FlxColor}> = [];
	var isDragging:Bool = false;
	final barSize:Int = FlxG.height - 78;
	final w = 10;

	public function new(totalHeight:Float, conductor:Conductor, segments:Array<
		{startTime:Float, startY:Float, stepCrochet:Float}>, playback:Song)
	{
		super();
	...
	public function removeBookmark(time:Float)
	{
		...
		if (bookmarks.length < oldLength) redrawBar();
	}
	...
	function timeToY(time:Float):Float
	{
		...
		for (i in 0...segments.length)
		{
			...
			if (time < nextStart) return seg.startY + ((time - seg.startTime) / seg.stepCrochet * LevelEditor.LANE_HEIGHT);
		}
	...
	function yToTime(y:Float):Float
	{
		...
		for (i in 0...segments.length)
		{
			...
			if (y < nextY) return seg.startTime + ((y - seg.startY) / LevelEditor.LANE_HEIGHT) * seg.stepCrochet;
		}
