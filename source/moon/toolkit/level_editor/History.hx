...
class History
{
	...
	static function pushNotePlace(n:NoteStruct):Void push(NotePlace(cloneNote(n)));

	static function pushNoteDelete(n:NoteStruct):Void push(NoteDelete(cloneNote(n)));
	...
	static function pushEventPlace(e:EventStruct):Void push(EventPlace(cloneEvent(e)));

	static function pushEventDelete(e:EventStruct):Void push(EventDelete(cloneEvent(e)));
	...
	private static function invert(action:EditorAction):EditorAction
	{
		return switch (action)
		{
			...
			case NoteDelete(n):
				NotePlace(n);
			case NoteDuration(n, old, now_):
				NoteDuration(n, now_, old);
			case EventPlace(e):
				EventDelete(e);
			case EventDelete(e):
				EventPlace(e);
	...
	private static function apply(action:EditorAction):Void
	{
		...
		switch (action)
		{
			...
			case NoteDelete(n):
				LevelEditor.chart.content.notes = LevelEditor.chart.content.notes.filter(
					existing -> !(Math.abs(existing.time - n.time) < 0.01 && existing.data == n.data && existing.lane == n.lane)
				);
			...
			case NoteDuration(n, _, newDur):
				...
				{
					if (Math.abs(existing.time - n.time) < 0.01 && existing.data == n.data && existing.lane == n.lane)
					{
			...
			case EventDelete(e):
				LevelEditor.chart.events = LevelEditor.chart.events.filter(existing -> !(existing.tag == e.tag && Math.abs(existing.time - e.time) < 0.01));
				ed.removeEventSpr(e);
		}
	}

	static function cloneNote(n:NoteStruct):NoteStruct return {
		time: n.time,
		data: n.data,
		lane: n.lane,
		type: n.type,
	...
	static function cloneEvent(e:EventStruct):EventStruct return {
		tag: e.tag,
		values: e.values,
		time: e.time,
		lane: e.lane
