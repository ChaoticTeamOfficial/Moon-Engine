...
class EditorTransition
{
	...
	public static function transitionToEditor(playState:PlayState):Void
	{
		...
		// Fade out HUD first
		FlxTween.tween(playState.camHUD, {
			alpha: 0
		}, 0.3, {
		...
		FlxTween.tween(playState.camGAME, {
			...
			onComplete: (_) ->
			{
				FlxG.switchState(() -> new LevelEditor(PlayState.songData.song, PlayState.songData.difficulty, PlayState.songData.mix));
