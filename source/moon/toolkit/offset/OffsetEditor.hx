...
class OffsetEditor extends FlxState
{
    ...
    override public function create():Void
    {
        ...
        final gs:Int = 64;
        for (gx in 0...Std.int(grid.width / gs) + 1)
            grid.pixels.fillRect(new openfl.geom.Rectangle(gx * gs, 0, 1, grid.height), gc);
        for (gy in 0...Std.int(grid.height / gs) + 1)
            grid.pixels.fillRect(new openfl.geom.Rectangle(0, gy * gs, grid.width, 1), gc);
        grid.dirty = true;
        grid.active = false;
        grid.x = -FlxG.width;
        grid.y = -FlxG.height;
    ...
    override public function update(elapsed:Float):Void
    {
        ...
        if (!uiFocused)
        {
            if (FlxG.keys.justPressed.SPACE && selectedAnim != '')
                char.playAnim(selectedAnim, true);

            if (FlxG.keys.justPressed.G)
                _toggleGhost(!ghostEnabled);
    ...
    private function _buildLeftPanel():Void
    {
        ...
        animList.onChange = _ -> { if (!suppressCallbacks) _onAnimSelected(); };
        ...
        animOffsetX.onChange = _ -> { if (!suppressCallbacks) _applyAnimOffset(); };
        animOffsetY.onChange = _ -> { if (!suppressCallbacks) _applyAnimOffset(); };
        ...
        playBtn.onClick = _ -> { if (selectedAnim != '') char.playAnim(selectedAnim, true); };
        ...
        animPrefixField.onChange = _ -> { _syncAnimDataFromUI(); };
        ...
        animIndicesField.onChange = _ -> { _syncAnimDataFromUI(); };
        ...
        animFpsField.onChange = _ -> { _syncAnimDataFromUI(); };
        ...
        animLoopedToggle.onChange = _ -> { _syncAnimDataFromUI(); };
        ...
        animFinishField.onChange = _ -> { _syncAnimDataFromUI(); };
        ...
        ghostToggle.onChange = _ -> { if (!suppressCallbacks) _toggleGhost(ghostToggle.selected); };
        ...
        ghostAnimDrop.onChange = _ -> {
            if (!suppressCallbacks && ghostAnimDrop.selectedItem != null)
                setGhostAnim(ghostAnimDrop.selectedItem.text, Std.int(ghostFrameField?.value ?? 0));
        };
        ...
        ghostFrameField.onChange = _ -> {
            if (!suppressCallbacks && ghostAnimName != '')
                setGhostAnim(ghostAnimName, Std.int(ghostFrameField.value));
        };
    ...
    private function _buildRightPanel():Void
    {
        ...
        camOffsetXField.onChange = _ -> { if (!suppressCallbacks) _applyCamOffsets(); };
        camOffsetYField.onChange = _ -> { if (!suppressCallbacks) _applyCamOffsets(); };
        ...
        extraOffsetXField.onChange = _ -> { if (!suppressCallbacks) _applyExtraOffsets(); };
        extraOffsetYField.onChange = _ -> { if (!suppressCallbacks) _applyExtraOffsets(); };
        ...
        resetBtn.onClick = _ -> { camGame.scroll.set(0, 0); };
    ...
    private function _populateAnimList():Void
    {
        ...
        {
            animList.dataSource.add({text: anim.name});
            ghostAnimDrop.dataSource.add({text: anim.name});
        }
    ...
    {
        for (a in char.data.animations)
            if (a.name == animName) return a;
        return null;
    }
    ...
    private function _applyAnimOffset():Void
    {
        ...
        if (ghostEnabled && ghostAnimName == selectedAnim)
            setGhostAnim(ghostAnimName, Std.int(ghostFrameField?.value ?? 0));
    }
    ...
    private function _applyCamOffsets():Void
        char.camOffsets = [camOffsetXField.value, camOffsetYField.value];
    ...
    private function _save():Void
    {
        ...
        d.overrideAnims = (rawOverride.trim() == '') ? []
            : [for (s in rawOverride.split(',')) s.trim()].filter(s -> s != '');
    ...
    private function _inlineNumber(parent:VBox, min:Float, max:Float, step:Float, def:Float):NumberStepper
    {
        ...
        ns.min = min; ns.max = max; ns.step = step; ns.value = def;
