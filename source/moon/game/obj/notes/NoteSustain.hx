...
class NoteSustain extends TiledSprite
{
	...
	override public function update(dt:Float):Void
	{
		...
		if (parent.state == CHART_EDITOR) expectedHeight = editorHeight;
		else
		{
		...
		if (downscroll) this.y -= height;

		this.flipY = downscroll;

		if (animation.curAnim != null && animation.curAnim.frameRate > 0 && animation.curAnim.frames.length > 1) animation.update(dt);
	...
	@:noCompletion
	public function set_parent(parentNote:Note):Note
	{
