...
class EventRegistry
{
	 ...
	 */
	public static function isHardcoded(tag:String):Bool return eventMap.exists(tag);

	/**
	 * Get all hardcoded event tags.
	 */
	public static function getHardcodedTags():Array<String> return[for (key in eventMap.keys()) key];
	...
	public static function executeEvent(game:PlayState, event:MoonEvent):Void
	{
		...
		if (eventClass != null)
		{
			...
			trace('[EVENT-REGISTRY] Unknown event: ${event.tag}', "WARNING");
	}
		...
		{name:String, description:String, category:GridType}>
	{
		...
		if (eventClass != null) return Type.createInstance(eventClass, [
			null,
			new MoonEvent(tag, {
			})
		]).getEditorData();
