...
class OLDPauseScreen extends FlxSubState
{
	...
	public function new(camera:FlxCamera)
	{
		...
		FlxTween.tween(bestGroup, {
			x: center - bestGroup.width / 2
		}, tweenDur, {
			ease: FlxEase.expoOut
		});
		for (woah in [songName, np]) FlxTween.tween(woah, {
			x: 55 + extraDist
		}, tweenDur, {
			ease: FlxEase.expoOut
		});

		for (ye in [paused, pausedL, oppIcon]) FlxTween.tween(ye, {
			x: 55
		}, tweenDur, {
			ease: FlxEase.expoOut
		});
	...
	function changeSelection(change:Int = 0):Void
	{
		...
		for (i in 0...pauseItems.members.length) pauseItems.members[i].selected = i == curSelected;
	...
	public function prepareToClose(?pressedEsc:Bool = false)
	{
		...
		{
			for (bg in [backGradient, back]) FlxTween.tween(bg, {
				alpha: 0
			}, pf.conductor.crochet / 1000 * 2);
			for (item in slideOutItems) FlxTween.tween(item, {
				x: item.x - 700
			}, pf.conductor.crochet / 1000, {
				ease: FlxEase.expoIn
			});
		});
