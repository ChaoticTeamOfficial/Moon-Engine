...
class NoteTypeRegistry
{
	...
	public static function isHardcoded(type:String):Bool return typeMap != null && typeMap.exists(type);

	public static function getHardcodedNames():Array<String> return[for (k in typeMap.keys()) k];
	...
	{
		if (getCls(type) != null) return getCls(type).getEditorData();
	...
	{
		if (getCls(type) != null) return getCls(type).getEditorFields();
	...
	{
		if (getCls(type) != null) return getCls(type).processValues(raw);
		return raw;
	}
	...
	private static function getCls(type:String, ?game:PlayState, ?note:Note)
	{
		...
		if (cls != null) return Type.createInstance(cls, [game, note]);
		return null;
	}
}
