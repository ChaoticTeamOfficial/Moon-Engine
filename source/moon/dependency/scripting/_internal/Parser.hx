...
class Parser
{
	...
	**/
	...
	public function new()
	{
		...
		for (i in 0...priorities.length) for (x in priorities[i])
		{
			opPriority.set(x, i);
	...
	{
		if (!resumeErrors) throw new Error(err, pmin, pmax, origin, line);
	}
	...
	function makeBinop(op, e1, e)
	{
		...
		return switch (expr(e))
		{
			case EBinop(op2, e2, e3):
				...
				if (
					delta < 0
					|| (delta == 0 && !opRightAssoc.exists(op))
				) mk(EBinop(op2, makeBinop(op, e1, e2), e3), pmin(e1), pmax(e3)); else mk(EBinop(op, e1, e), pmin(e1), pmax(e));
				...
				while (true)
				{
					...
					switch (tk)
					{
						case TId("case"):
							...
							c.expr = if (exprs.length == 1) exprs[0]; else if (exprs.length == 0) mk(
								EBlock([]),
								tokenMin,
								tokenMin
						...
						case TId("default"):
							...
							def = if (exprs.length == 1) exprs[0]; else if (exprs.length == 0) mk(
								EBlock([]),
								tokenMin,
								tokenMin
							); else mk(EBlock(exprs), pmin(exprs[0]), pmax(exprs[exprs.length - 1]));
				...
				case Expr(depth, quoteChar):
					...
					if (quoteChar == null)
					{
						...
						else if (c == '}'.code)
						{
							...
							var oldPreprocStack:Array<
								{r:Bool}> = preprocStack.copy();
							...
							var e:Expr = parseString(b.toString(), origin, p1 + i - b.length);
		...
		switch (expr(e))
		{
			...
			case EBinop(op, e1, e2):
				...
				return switch (op)
				{
					...
					case "||": evalPreproCond(e1) || evalPreproCond(e2);
					case ">=":
						(preprocValue(value1) ?? value1) >= (preprocValue(value2) ?? value2);
					case ">":
						(preprocValue(value1) ?? value1) > (preprocValue(value2) ?? value2);
					case "<=":
						(preprocValue(value1) ?? value1) <= (preprocValue(value2) ?? value2);
	...
	static function get_preprocessorValues():Map<String, Dynamic>
	{
		// TODO: add a macro for reading preprocessors
		return [];
	}
}
