...
import moon.dependency.scripting._internal.references.*;
...
class Tools
{
	...
	public static function map(e:Expr, f:Expr->Expr)
	{
		var edef = switch (expr(e))
		{
			...
			case EFinal(n, t, e):
				EFinal(n, t, if (e != null) f(e) else null);
			case EParent(e):
				EParent(f(e));
			case EBlock(el):
			...
			case ESwitch(e, cases, def):
				ESwitch(f(e), [for (c in cases) {
					values: [for (v in c.values) f(v)],
					expr: f(c.expr)
				}], def == null ? null : f(def));
			case EMeta(name, args, e):
	...
	public static function objectRef(object:Dynamic):Null<Reference<Dynamic>>
	{
		...
		{
			case TClass(c):
				return new DefaultReference.ClassReference(object);
			default:
		}

		/*if (Std.isOfType(object, Enum))
			{
				return new EnumReference(object);
			}
			else */
		if (Std.isOfType(object, Class))
		{
			return new StaticClassReference(object);
		}

		return new DefaultReference(object);
	}
