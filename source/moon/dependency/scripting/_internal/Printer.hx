...
class Printer
{
	...
	public function modulesToString(m:Array<ModuleDecl>):String
	{
		...
		for (module in m)
		{
			switch (module)
			{
				...
				case DField(f):
					output += "\n";
					output += classFieldsToString([f], true);
			}
	...
	public static function errorToString(e:Expr.Error, includePosInfo:Bool = true):String
	{
		...
		{
			case EInvalidChar(c):
				"Invalid character: '" + (StringTools.isEof(c) ? "EOF" : String.fromCharCode(c)) + "' (" + c + ")";
			case EUnexpected(s):
				"Unexpected token: \"" + s + "\"";
			...
			case EInvalidPreprocessor(str):
				"Invalid preprocessor (" + str + ")";
			...
			case EInvalidAccess(f):
				"Invalid access to field " + f;
			case EInvalidModule(m):
				"Invalid module: " + m;
			case EBlacklistedModule(m):
				"Blacklisted module: " + m;
			case EBlacklistedField(m):
				"Blacklisted field: " + m;
			case EInvalidArgCount(f, expected, given):
				'Provided arguments are fewer than the required function parameters. Got $given, required $expected' + f;
			case EExceedArgsCount(f, allowed, passed):
				'Provided arguments exceeds the allowed function parameter count. Passed $passed, allowed $allowed' + f;
			case EPurgedFunction(f):
				"Invalid access to purged function (did it throw an uncaught exception earlier?): " + f;
			case ENullObjectReference(f):
				"Invalid reference to field of a null object: " + f;
			case EInvalidInStaticContext(v):
				"Invalid field access from static context: " + v;
			case EInvalidScriptedFnAccess(f):
				"Invalid function access to scripted class: " + f;
			case EInvalidScriptedVarGet(v):
				"Invalid variable retrieval to scripted class: " + v;
			case EInvalidScriptedVarSet(v):
				"Invalid variable assignment to scripted class: " + v;
			case EInvalidFinalSet(f):
				"Invalid final field assignment: " + f;
			case EInvalidPropGet(p):
				"Cannot access property " + p + " for reading";
			case EInvalidPropSet(p):
				"Cannot access property " + p + " for writing";
			case EPropVarNotReal(p):
				"Cannot access property " + p + " because it is not a real variable";
			...
			case EClassUnresolvedSuperclass(c, r):
				'Unresolved superclass $c (reason: $r)';
			// TODO: Do we need to distinguish these?
			case EScriptCallThrow(v):
				"Script threw an exception: " + v;
			case EScriptThrow(v):
				"User script threw an exception: " + v;
			case ECustom(msg):
				msg;
		};
