...
enum ExprDef
{
	...
	ESwitch(e:Expr, cases:Array<
		{values:Array<Expr>, expr:Expr}>, ?defaultExpr:Expr);
...
typedef Metadata = Array<
	{name:String, params:Array<Expr>}>;
...
enum CType
{
	...
	CTAnon(fields:Array<
		{name:String, t:CType, ?meta:Metadata}>);
	CTParent(t:CType);
	CTOpt(t:CType);
...
typedef ClassImport =
{
	...
	/**
	 * The underlying reference that was imported.
	 * Will be `null` if the underlying object was BLACKLISTED.
	 */
	var ?ref:moon.dependency.scripting._internal.Reference<Dynamic>;
}
