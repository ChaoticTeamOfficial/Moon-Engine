...
class MoonEvent extends MoonScript
{
	...
	public function new(tag:String, values:Dynamic)
	{
		...
		if (!EventRegistry.isHardcoded(tag) && Paths.exists('data/events/$tag.hx')) load('data/events/$tag.hx');
	 ...
	 */
	public function exec() if (valid) call('onExecute', [values]);
	...
	@:noCompletion
	public function set_PRESET_VARIABLES(vars)
	{
