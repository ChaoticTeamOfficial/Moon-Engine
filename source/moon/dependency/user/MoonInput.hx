...
class MoonInput
{
	public static var binds:Map<String, Array<Array<Dynamic>>> = [
		// Gameplay Keybinds
		'LEFT' => [
	...
	public static function checkBind(rawBind:MoonKeys, inputState:FlxInputState):Bool
	{
		if (Global.allowInputs)
		{
			...
			if (!binds.exists(bind)) return false;

			// for keyboards
			final thisBind:Array<FlxKey> = cast binds.get(bind)[0];
			for (key in thisBind) if (FlxG.keys.checkStatus(key, inputState)) return true;

			// for controllers
			final thisControllerBind:Array<FlxPad> = cast binds.get(bind)[1];
			if (FlxG.gamepads.lastActive != null) for (key in thisControllerBind) if (FlxG.gamepads.lastActive.checkStatus(key, inputState)) return true;
		}
		return false;
	}
	...
	{
		if
			((MoonSettings.save.data.binds != null)
				&& (Lambda.count(MoonSettings.save.data.binds) == Lambda.count(binds))
	...
	public static function justPressed(bind:MoonKeys):Bool return checkBind(bind, JUST_PRESSED);

	public static function pressed(bind:MoonKeys):Bool return checkBind(bind, PRESSED);

	public static function released(bind:MoonKeys):Bool return checkBind(bind, JUST_RELEASED);
}
