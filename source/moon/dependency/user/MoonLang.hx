...
class MoonLang
{
    ...
    static function scan():Void
    {
        ...
        if (available.length == 0)
            trace('[LANG] No locale files found in "data/locales"!', "WARNING");
        else
     ...
     */
    static function loadFromSettings():Void
        load(_codeFromName(MoonSettings.callSetting('Language') ?? '') ?? Constants.FALLBACK_LANG);
    ...
    static function load(code:String):Void
    {
        ...
        if (code == Constants.FALLBACK_LANG)
     ...
     */
    static inline function settingName(name:String):String
        return get('setting.$name.name', name);

    /**
     * Returns the localized description for a setting.
     * Falls back to `fallbackDesc` if no translation exists.
     */
    static inline function settingDesc(name:String, ?fallbackDesc:String):String
        return get('setting.$name.desc', fallbackDesc ?? '');

    /**
     * Returns the localized display name for a settings category.
     * Falls back to the category string itself.
     */
    static inline function category(name:String):String
        return get('category.$name', name.toUpperCase());

    /**
     * Returns the display names of all available locales, in scan order.
     * Used to populate the Languages in settings.
     */
    static function getDisplayNames():Array<String>
        return [for (l in available) l.name];

    /**
     * Returns the locale codes for all available locales, in scan order.
     */
    static function getCodes():Array<String>
        return [for (l in available) l.code];
    ...
    {
        for (l in available)
            if (l.name == displayName) return l.code;
        return null;
    }
    ...
    private static function _loadVanilla(code:String, into:Map<String, String>):Void
    {
        ...
        catch (e) { trace('[LANG] Failed to parse vanilla locale "$code": $e', "ERROR"); }
        #else
    ...
    private static function _mergeStrings(data:Dynamic, into:Map<String, String>):Void
    {
        ...
        for (key in Reflect.fields(data.strings))
            into.set(key, Std.string(Reflect.field(data.strings, key)));
    }
}