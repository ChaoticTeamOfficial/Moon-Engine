...
/**
 * A modified `MoonSprite` that supports filters.
 * The name's pretty much self-explanatory.
 * FlxFilteredSprite originally by Funkin' Crew.
 */
@:nullSafety
@:access(openfl.geom.Rectangle)
@:access(openfl.filters.BitmapFilter)
@:access(flixel.graphics.frames.FlxFrame)
class FilteredSprite extends MoonSprite
{
	@:noCompletion
	var _renderer:FlxAnimateFilterRenderer = new FlxAnimateFilterRenderer();
	@:noCompletion
	var _filterMatrix:FlxMatrix = new FlxMatrix();
	...
	@:nullSafety(Off) @:noCompletion
	var _blankFrame:FlxFrame;
	@:nullSafety(Off)
	...
	function filterFrame()
	{
		...
		if (filters != null && filters.length > 0)
		{
			...
			@:nullSafety(Off)
			_blankFrame.parent.bitmap = _renderer.applyFilter(
				_blankFrame.parent.bitmap,
				_filterBmp1,
				_filterBmp2,
				frame.parent.bitmap,
				filters,
...
class FlxAnimateFilterRenderer
{
	...
	@:noCompletion
	function setRenderer(renderer:DisplayObjectRenderer, rect:Rectangle)
	{
		@:privateAccess
		if (true)
		{
	...
	public function applyFilter(target:BitmapData = null, target1:BitmapData = null, target2:BitmapData = null, bmp:BitmapData, filters:Array<BitmapFilter>, rect:Rectangle, bmpRect:Rectangle)
	{
		...
		var bitmap2 = (target1 == null) ? new BitmapData(
			Math.ceil(rect.width * 1.25),
			Math.ceil(rect.height * 1.25),
			true,
			0
		) : target1, bitmap3 = (target2 == null) ? bitmap2.clone() : target2;
