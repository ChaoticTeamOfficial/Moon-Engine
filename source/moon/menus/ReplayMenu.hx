...
class ReplayMenu extends FlxSubState
{
	...
	public function new()
	{
		...
		infoBoxGroup = new FlxSpriteGroup(); add(infoBoxGroup);

		var infoBoxBG = new MoonSprite().makeGraphic(350, Std.int(FlxG.height * 0.85), FlxColor.BLACK);
		infoBoxGroup.add(infoBoxBG);
		infoBoxGroup.screenCenter(); infoBoxGroup.x = FlxG.width - infoBoxGroup.width - 8;

		var placeholder = new MoonSprite().loadGraphic(Paths.image('menus/replay/placeholder'));
		placeholder.setGraphicSize(320); placeholder.updateHitbox(); placeholder.x += 15; placeholder.y += 32;
		infoBoxGroup.add(placeholder);
		
		var infoDivider = new MoonSprite().makeGraphic(320, 4, FlxColor.GRAY);
		infoDivider.x = 15; infoDivider.y += placeholder.height + 104; infoDivider.antialiasing = true;
		infoBoxGroup.add(infoDivider);

		songName = new FlxText();
		songName.x = 15; songName.y += placeholder.height + 48;
		songName.setFormat(Paths.font('phantomuff/full.ttf'), 24, CENTER);
        songName.antialiasing = true; infoBoxGroup.add(songName);

		chartName = new FlxText();
		chartName.x = 15; chartName.y = placeholder.height + 78;
		chartName.setFormat(Paths.font('phantomuff/full.ttf'), 16, CENTER);
		chartName.color = FlxColor.GRAY;
        chartName.antialiasing = true; infoBoxGroup.add(chartName);

		replayRank = new FreeplayRank();
        replayRank.x = 285; replayRank.y += placeholder.height + 4;
        replayRank.setRank('PERFECT-GOLD'); replayRank.updateHitbox();
	...
	function createButtons() {
		...
		playButton.x = 10; playButton.y += infoBoxGroup.height - playButton.height - 68;
		infoBoxGroup.add(playButton);
		
		editButton = new ReplayButton(165, 48, 'Edit Replay');
		editButton.x = 20 + playButton.width; editButton.y += infoBoxGroup.height - editButton.height - 68;
		infoBoxGroup.add(editButton);
		
		deleteButton = new ReplayButton(330, 48, 'Delete Replay', 0xFF600000); deleteButton.members[2].x = 84;
		deleteButton.x = 10; deleteButton.y += infoBoxGroup.height - deleteButton.height - 10;
		deleteButton.deselectColors[1] = 0xFFFF0000; deleteButton.setDeselect();
	...
	function changeSelection(change:Int = 0):Void
    {
		...
		if (replays[curSelected].acc != null) {
			...
			replayRank.setRank(rank); replayRank.updateHitbox();
		} else replayRank.visible = false;
	...
	function deleteReplay() {
		...
		if (replays.length == 0) { // just close the substate for now, we should add a proper "No Replays!" check later
			close();
			return;
		}
		changeSelection(-1);
