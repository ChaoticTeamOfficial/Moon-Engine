...
class Freeplay extends FlxSubState
{
	...
	public function new(character:String = 'bf')
	{
		...
		FlxTween.tween(weekBG, {
			x: FlxG.width - weekBG.width + 360,
			"skew.x": 5
		...
		{
			if ((beat % 2 == 0 || conductor.bpm < 120) && thisDJ.canDance) thisDJ.playAnim('idle', true);
	...
	public function getMixSonglist(week:String, preferredMix:String, diff:String):Array<SongBase>
	{
		...
		for (song in SongLibrary.get().weekSonglist(week)) if (song.mix == preferredMix && song.difficulty == diff) filtered.push(song);
	 ...
	 */
	public function changeCategory(delta:Int):Void
	{
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (FlxG.sound.music != null)
		{
			...
			try
			{
				if (
					...
					|| (FlxG.sound.music.fadeTween != null && !FlxG.sound.music.fadeTween.active)
		...
		if (FlxG.keys.justPressed.Q) changeCategory(-1);
		if (FlxG.keys.justPressed.E) changeCategory(1);
