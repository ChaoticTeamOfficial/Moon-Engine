...
class FreeplaySongSelector extends FlxGroup
{
    ...
    function playEnterAnimation():Void
    {
        ...
        for (i in 0...items.length)
        {
            ...
            FlxTween.tween(item,
                {lerpAlpha: isCenter ? 1 : item.targetAlpha, lerpScale: isCenter ? 1 : item.targetScale}, duration,
                {ease: ease, startDelay: delay}
            );
        }
    }
    ...
    public function playExitAnimation(?onComplete:Void->Void):Void
    {
        ...
        FlxTween.tween(disk, {"scale.x": 0, "scale.y": 0},
            EXIT_DURATION,
            {ease: FlxEase.backIn, onUpdate: _ -> disk.updateHitbox()});

        for (i in 0...items.length)
        {
            final delay = Math.abs(i - VISIBLE_RADIUS) * STAGGER_DELAY * 0.6;
            FlxTween.tween(items[i], {lerpAlpha: 0, lerpScale: 0},
                EXIT_DURATION * 0.6,
                {ease: FlxEase.backIn, startDelay: delay});
        }

        // Fade out auxiliary elements
        if (albumTitle.visible)
            FlxTween.tween(albumTitle, {alpha: 0}, EXIT_DURATION * 0.4);
        ...
        if (noSongsText.visible)
        {
            ...
            FlxTween.tween(noSongsText, {alpha: 0}, EXIT_DURATION * 0.3);
            FlxTween.tween(noSongsSubtext, {alpha: 0}, EXIT_DURATION * 0.3);
        }
    ...
    function diffSwitchAnim():Void
    {
        ...
        for (i in 0...items.length)
            if (items[i].lerpAlpha > 0.1) items[i].playDifficultySwitchEffect();
    ...
    function updateNoSongsVisibility():Void
    {
        if (songList.length == 0)
        {
            ...
            FlxTween.tween(noSongsText, {alpha: 1}, 0.5, {ease: FlxEase.circOut});
            FlxTween.tween(noSongsSubtext, {alpha: 1}, 0.5, {ease: FlxEase.circOut, startDelay: 0.15});
        }
        else
        {
            FlxTween.tween(noSongsText, {alpha: 0}, 0.3, {
                ease: FlxEase.circIn,
                onComplete: _ -> noSongsText.visible = false
            });
            FlxTween.tween(noSongsSubtext, {alpha: 0}, 0.3, {
                ease: FlxEase.circIn,
    ...
    public function getSelectedItem():FreeplaySongItem
        return items.length > 0 ? items[VISIBLE_RADIUS] : null;
    ...
    inline function isValidIndex(idx:Int):Bool
        return idx >= 0 && idx < songList.length;
    ...
    {
        final album = Paths.exists('images/menus/freeplay/albums/${chart.content.meta.album}.png')
            ? chart.content.meta.album
            : 'placeholder';
        ...
        if (curAlb != album)
        {
            ...
            if (!album.contains('placeholder'))
            {
                ...
                FlxTween.tween(albumTitle, {alpha: 1}, 0.35, {ease: FlxEase.circOut});
            }
            else
                albumTitle.visible = false;
        }
