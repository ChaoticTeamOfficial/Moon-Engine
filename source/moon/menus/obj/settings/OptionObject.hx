...
class OptionObject extends FlxSpriteGroup
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (selected && setting.type != INFO && (MoonInput.pressed(UI_LEFT) || MoonInput.pressed(UI_RIGHT)))
		{
			...
			if (holdTimer <= 0)
			{
				...
				if (direction != 0) changeValue(direction);
		...
		if (MoonInput.justPressed(ACCEPT) && setting.type == SELECTABLE && selected)
		{
			...
			{
				case 'Keybinds...':
					FlxG.state.openSubState(new Keybinds());
				case 'HUD Customization...':
					FlxG.state.openSubState(new JudgementsComboCustomize());
				case "Calculate Offset...":
					FlxG.state.openSubState(new Offset());
			}
		}
	}
	...
	public function changeValue(amount:Int)
	{
		...
		{
			case CHECKMARK:
				if (amount != 0) setting.value = !setting.value;

				value.text = (setting.value) ? "< On >" : "< Off >";

				if (amount != 0) Paths.playSFX('menus/settings/settings${setting.value ? "ON" : "OFF"}.wav');
			...
			case SELECTOR:
				...
				if (amount != 0) Paths.playSFX('menus/settings/settings$str.wav', 'sounds', true);
			...
			case SLIDER:
				...
				if (amount != 0) Paths.playSFX(
					'menus/settings/settingsMeterChange.wav',
					'sounds',
					true,
					FlxMath.remapToRange(setting.value, 0, 100, 0.5, 1.5)
				);
			...
			case UNCAP_SLIDER:
				...
				if (amount != 0) Paths.playSFX('menus/settings/settingsMeterChange.wav', 'sounds', true, FlxG.random.float(0.8, 1.2));

			case INFO:
				value.text = '( ${setting.defaultValue} )';
		...
		if (amount != 0)
		{
			...
			switch (setting.name)
			{
				case 'Window Resolution', 'Screen Mode':
					MoonSettings.updateWindow();
				case 'Language':
					MoonLang.loadFromSettings();
					if (Settings.current != null) Settings.current.refreshOptions();
			}

			if (PlayField.instance != null) PlayField.instance.settingsUpdate();
		}
	}
