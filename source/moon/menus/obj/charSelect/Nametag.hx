...
class Nametag extends MoonSprite
{
	...
	@:noCompletion
	public function set_character(character:String):String
	{
		if (this.character == character) return this.character;
		this.character = character;

		loadGraphic(
			(character != 'locked') ? Paths.image('$character/${character}Chill/nametag', 'characters') : Paths.image('menus/charSelect/lockedNametag')
		);
		this.shader = mosaic;
		TweenUtils.cancelTwn(mosaicTwn);

		mosaic.bSize = 32.0;
		mosaicTwn = FlxTween.tween(mosaic, {
			bSize: 1.0
		}, 0.4);
