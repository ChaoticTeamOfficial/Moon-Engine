...
class ReplayItem extends FlxSpriteGroup
{
	...
	public function new(x:Float, y:Float, data:String):Void
	{
		...
		else
		{
			...
			rankShadow.x += 430;
			rankShadow.y -= 28;
			rankShadow.setRank(rank);
			rankShadow.updateHitbox();
			add(rankShadow);
			rankShadow.color = FlxColor.BLACK;
		}
		add(rankDisplay);

		iconOpponent = new PixelIcon(4, 4, 'bf');
		iconOpponent.character = chartData.content.meta.opponents[0];
		iconOpponent.character = chartData.content.meta.opponents[0];
		add(iconOpponent);
		iconOpponent.setGraphicSize(48);
		iconOpponent.updateHitbox();
		iconPlayer = new PixelIcon(32, 16, 'bf');
		iconPlayer.character = chartData.content.meta.players[0];
		iconPlayer.character = chartData.content.meta.players[0];
		add(iconPlayer);
		iconPlayer.setGraphicSize(48);
		...
		var date:Date;
		if (replayData.replayCode != null) date = Date.fromTime(replayData.replayCode);
		else
