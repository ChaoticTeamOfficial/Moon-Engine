...
class ScrollingArts extends FlxSpriteGroup
{
	...
	public function doScrolling(index:Int)
	{
		...
		for (twn in [tween1, tween2, tween3]) TweenUtils.cancelTwn(twn);

		tween1 = FlxTween.tween(spr, {
			alpha: 1
		}, 0.5);
		tween2 = FlxTween.tween(spr, {
			x: (cool) ? spr.x - scrollAmmount : spr.x + scrollAmmount
		}, scrollLength);
		tween3 = FlxTween.tween(spr, {
			alpha: 0.0001
		}, 0.5, {
			startDelay: scrollLength - 0.5,
			onComplete: (_) -> doScrolling(FlxG.random.int(0, this.members.length - 1, [index]))
		});
	}
}
