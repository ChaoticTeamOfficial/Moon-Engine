...
class Title extends FlxTransitionableState
{
	...
	override public function create():Void
	{
		...
		for (i in 0...3)
		{
			...
			var awa = new FlxShapeCircle(0, 0, stuff[i], {
				thickness: 4,
		...
		conductor.onBeat.add((beat) ->
		{
			...
			if (!onTitle)
			{
				switch (beat)
				{
					...
					// biggie!!
					case 1:
						setTxt('The Funkin\' Crew');
					case 3:
						setTxt('presents');
					case 4:
						setTxt(true);
					case 5:
						setTxt('(NOT) In association\nwith');
					case 7:
						ctlogo.visible = true;
						FlxTween.tween(ctlogo, {
							"scale.x": 1,
							"scale.y": 1
						}, conductor.crochet / 1010, {
	...
	public function setTxt(?text:String = '', ?clear:Bool = false)
	{
		if (!onTitle)
		{
			...
			txTwn = FlxTween.tween(displayTxt, {
				y: displayTxt.y - 15
			}, 1.1, {
	...
	override public function update(elapsed:Float):Void
	{
		...
		for (obj in objects) obj.visible = onTitle;
	}
	...
	private function codePress(input:Int)
	{
		...
		if (input == codeKeys[codePos])
		{
			...
			if (codePos >= codeKeys.length)
			{
				...
				if (vidTimer != null && vidTimer.active) vidTimer.cancel();
	...
	function updateVis() @:privateAccess backVis.setAudioSource(cast FlxG.sound.music._channel.__audioSource);
	...
	private function setupTransition()
	{
		...
		FlxTransitionableState.defaultTransIn = new TransitionData(FADE, FlxColor.BLACK, transitionDuration, new FlxPoint(-2, -1), {
			asset: diamond,
			width: 32,
			height: 32
		}, new FlxRect(-1, 0, FlxG.width, FlxG.height));
		FlxTransitionableState.defaultTransOut = new TransitionData(FADE, FlxColor.BLACK, transitionDuration, new FlxPoint(2, -1), {
			asset: diamond,
			width: 32,
			height: 32
		}, new FlxRect(-1, 0, FlxG.width, FlxG.height));
