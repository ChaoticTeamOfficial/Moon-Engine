...
class Offset extends FlxSubState
{
	...
	public function new()
	{
		...
		var instructText = new FlxText(0, calibrate.y + 60, FlxG.width, "Tap space to the beat to calibrate your game!\nyour offset will be calibrated to the beat you're tapping.");
		...
		FlxTween.tween(instructText, {alpha: 1}, 1, {startDelay: 0.7, ease: FlxEase.quadInOut});
		...
		FlxTween.tween(goback, {alpha: 0.5}, 2, {startDelay: 2, ease: FlxEase.quadInOut});
		...
		bar.value = 0;
		bar.alpha = 0.0001;
		FlxTween.tween(bar, {alpha: 1}, 2, {startDelay: 2, ease: FlxEase.quadInOut});
        ...
        FlxTween.tween(offsetDebug, {alpha: 0.6}, 2, {startDelay: 0.4, ease: FlxEase.quadInOut});
		...
		add(centerIndicator);
		FlxTween.tween(centerIndicator, {alpha: 0.6}, 1, {startDelay: 3, ease: FlxEase.quadInOut});
	...
	override public function update(elapsed:Float)
	{
		...
		for(line in lines)
			if(line != null)
				cast(line, MoonSprite).alpha = FlxMath.lerp(cast(line, MoonSprite).alpha, linesAlpha, elapsed * 2);

		centerIndicator.scale.x = centerIndicator.scale.y = FlxMath.lerp(centerIndicator.scale.x, 1, elapsed * 16);

		if(MoonInput.justPressed(ACCEPT) && allowPress)
			calculateOffset();
