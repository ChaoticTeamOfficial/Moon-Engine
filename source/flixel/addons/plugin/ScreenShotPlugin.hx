...
class ScreenShotPlugin extends flixel.FlxBasic
{
	...
	{
		flashColor = v;
		if (current != null && current.flashBitmap != null) current.flashBitmap.bitmapData = new BitmapData(lastWidth, lastHeight, true, v);
		return flashColor;
	}
	...
	{
		if (FlxG.keys.anyJustPressed(screenshotKeys) && enabled) screenshot();
	}
	...
	private function screenshot():Void
	{
		...
		if (!sys.FileSystem.exists('./${screenshotPath}/')) sys.FileSystem.createDirectory('./${screenshotPath}/');
		...
		FlxTween.tween(screenshotSprite, {
			alpha: 0
		}, 0.5, {
			startDelay: .5
		});
	...
	override public function destroy():Void
	{
		...
		@:privateAccess
		for (parent in [screenContainer, flashSprite, screenshotSprite]) for (child in parent.__children) parent.removeChild(child);
	...
	override public function toString():String return "ScreenshotPlugin instance";
...
enum abstract FileFormatOption(String) from String
{
	...
	public function returnEncoder():Any
	{
		...
		{
			case JPEG:
				new openfl.display.JPEGEncoderOptions(ScreenShotPlugin.jpegQuality);
			default:
