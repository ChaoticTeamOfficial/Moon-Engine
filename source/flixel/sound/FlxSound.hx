...
class FlxSound extends FlxBasic
{
	...
	public function loadEmbedded(EmbeddedSound:FlxSoundAsset, Looped:Bool = false, AutoDestroy:Bool = false, ?OnComplete:Void->Void):FlxSound
	{
		...
		{
			if (FlxG.assets.exists(EmbeddedSound, SOUND)) _sound = FlxG.assets.getSoundUnsafe(EmbeddedSound);
			else
	...
	function updateTransform():Void
	{
		try
		{
			...
			if (_channel != null && _channel.soundTransform != null) _channel.soundTransform = _transform;
		}
		catch (e)
		{
		}
