...
{	
	background.adjustGroupColor(background.opponents, {brightness: -33, hue: -32, contrast: -23, saturation: 0});
	
	for(character in background.players.members)
		addShader(character, -23, 12, 7, 0, 0xFFffe346, 50);
	
	for(character in background.spectators.members)
		addShader(character, -30, -9, -4, 0, 0xFFc8b023, 0);
}
