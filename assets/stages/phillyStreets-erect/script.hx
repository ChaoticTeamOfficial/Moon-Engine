...
function createCar():MoonSprite
{
	...
	for (i in 1...5) car.animation.addByPrefix('car' + i, 'car' + i + '0', 24, false);
...
function onPostUpdate(elapsed:Float)
{
	...
	rainShader.intensity =
		game.playField.inCountdown
		|| game.playField.inCutscene ? intensityRange[0] : FlxMath.remapToRange(
			game.conductor.time,
			0,
			game.playField.playback.inst[0]?.length ?? 0,
			intensityRange[0],
			intensityRange[1]
		);
...
function onBeat(beat:Int)
{
	...
	if (beat == lastLightChangeBeat + lightChangeInterval) changeTrafficLights(beat);
	...
	// car 2 (backward)
	if (FlxG.random.bool(10) && beat != lastLightChangeBeat + lightChangeInterval && car2Interruptable && !lightsStop) driveCarBackward(car2);
}
...
function changeTrafficLights(beat:Int)
{
	...
	if (!lightsStop && carWaiting) finishCarAtLights(car1);
}
...
function driveCarGeneric(car:MoonSprite, stopAtLights:Bool, backward:Bool)
{
	...
	switch (variant)
	{
		...
		case 2:
			extraOffset = [20, 42];
			duration = FlxG.random.float(0.6, 1.2); // why this bitch fast
		case 3:
			extraOffset = [30, 50];
	...
	FlxTween.angle(car, startRot, endRot, duration, {
...
function finishCarAtLights(car:MoonSprite)
{
	...
	FlxTween.angle(car, -5, 18, duration, {
		ease: FlxEase.sineIn
	});
	FlxTween.quadPath(car, path, duration, true, {
		ease: FlxEase.sineIn,
